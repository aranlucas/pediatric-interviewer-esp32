...
static bool touch_i2c_write_read(uint8_t driver_addr, uint8_t *write_buf, uint32_t write_len, uint8_t *read_buf, uint32_t read_len)
{
    ...
    if (g_touch_i2c->requestFrom(driver_addr, read_len) != read_len) {
        Serial.println("The I2C read fails. - I2C Read\r\n");
        return false;
    }
    return g_touch_i2c->readBytes(read_buf, read_len) == read_len;
}
...
{
    // A released touch must not leave the previous coordinates active.
